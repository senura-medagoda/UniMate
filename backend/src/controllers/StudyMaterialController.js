    ...
    try {
      if (req.files && req.files.length > 0) {
        const uploadPromises = req.files.map(file =>
          uploadBufferToCloudinary(file.buffer, 'study-materials', 'auto')
        );
        const results = await Promise.all(uploadPromises);
        results.forEach(result => fileUrls.push(result.secure_url));
      } else if (req.file) {
        const result = await uploadBufferToCloudinary(req.file.buffer, 'study-materials', 'auto');
        fileUrls.push(result.secure_url);
      }
    } catch (uploadError) {
      console.error('Study material upload error:', uploadError);
