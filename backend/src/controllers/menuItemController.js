...
export const createMenuItem = async (req, res) => {
    try {
        ...
        if (req.files && req.files.image && req.files.image[0]) {
            try {
                const result = await uploadBufferToCloudinary(req.files.image[0].buffer, 'menu-items');
                menuData.image = result.secure_url;
                console.log('Image uploaded successfully to Cloudinary:', result.secure_url);
            } catch (imageError) {
                console.error('Image upload error:', imageError);
                menuData.image = 'https://via.placeholder.com/800x600/f3f4f6/9ca3af?text=Upload+Failed';
            }
        }
        ...
        if (req.files && req.files.images && req.files.images.length > 0) {
            try {
                const imagePromises = req.files.images.map(file =>
                    uploadBufferToCloudinary(file.buffer, 'menu-items')
                );
    ...
    try {
        ...
        if (req.files && req.files.image && req.files.image[0]) {
            try {
                ...
                console.error('Image upload error:', imageError);
                updates.image = 'https://via.placeholder.com/800x600/f3f4f6/9ca3af?text=Upload+Failed';
            }
        }
