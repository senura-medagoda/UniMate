...
function SA_Nav() {
  ...
  const navItems = [
    { name: 'Dashboard', path: '/systemadmin-dash', icon: <BarChart3 className="w-4 h-4" /> },
    { name: 'Students', path: '/systemadmin-students', icon: <GraduationCap className="w-4 h-4" /> },
    { name: 'Admins', path: '/systemadmin-admins', icon: <Users className="w-4 h-4" /> },
    { name: 'Reports', path: '/systemadmin-reports', icon: <BarChart3 className="w-4 h-4" /> }
  ];
    ...
    >
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          >
            <Link to="/systemadmin-dash" className="flex items-center">
              <img 
                src="/Logo.png" 
                alt="UniMate Logo" 
          ...
          <div className="hidden lg:flex items-center space-x-8">
            {navItems.map((item, index) => (
              ...
              >
                <Link
                  to={item.path}
                  className={`font-medium transition-colors duration-200 relative group flex items-center gap-2 ${
                    isActive(item.path) 
                      ? 'text-orange-600' 
            ...
            >
              <div className="dropdown dropdown-end">
                ...
                <ul tabIndex={0} className="mt-3 p-2 shadow menu menu-compact dropdown-content bg-white rounded-box w-52 border border-gray-200">
                  <li>
                    <Link to="/systemadmin-profile" className="flex items-center gap-2">
