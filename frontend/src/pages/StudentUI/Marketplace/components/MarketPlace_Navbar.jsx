...
const MarketPlace_Navbar = ({ user, setUser }) => {
  ...
  const logout=()=>{
    ...
    localStorage.removeItem('marketplaceCart') // Clear cart from localStorage
    setToken('')
    setCartItem({})
    
    // Clear user data if setUser is available
    if (setUser) {
      setUser(null)
    }
  }
  ...
  return (
    <div className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
     ...
     <Link to ='/std-dash'><img src={assets.logoMain} className='w-25 h-6' alt="" /></Link> 
     ...
     <div className='flex items-center gap-6' >
      ...
      <div className='group relative' >
        ...
        <img onClick={()=> token ? null : navigate('/login-std')} src={assets.profile_icon} className={`w-5 cursor-pointer ${
          isHomePage && !isScrolled ? 'brightness-0 invert' : ''
        }`} alt="" />
