...
const JPA_Login = () => {
  ...
  const { login, loading, isAuthenticated } = useJPAuth();

  // Check if already logged in
  useEffect(() => {
    if (isAuthenticated()) {
      console.log('JP Admin already logged in, redirecting to dashboard');
      const from = location.state?.from?.pathname || '/jpadmin-dash';
