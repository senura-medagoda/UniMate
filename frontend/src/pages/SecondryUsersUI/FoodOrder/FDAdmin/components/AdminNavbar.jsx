      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          ...
          <div className="flex items-center">
            <Link to="/food/admin/dashboard" className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-gray-900">Food Admin</h1>
            ...
            {navItems.map((item) => (
              <Link
                ...
                onClick={() => {
                  ...
                  if (item.path === '/food/admin/orders') {
              ...
              {isProfileOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-50">
                  <div className="px-4 py-2 border-b border-gray-100">
                    ...
                    <p className="text-xs text-orange-600">{admin?.department}</p>
                  </div>
                  <Link
                    to="/food/admin/profile"
            ...
            <div className="space-y-2">
              {navItems.map((item) => (
                <Link
                  key={item.name}
                  to={item.path}
                  className="flex items-center space-x-3 px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-orange-600 hover:bg-orange-50"
                  onClick={() => {
                    console.log('Mobile navigation to:', item.path);
                    setIsMenuOpen(false);
                    // Force navigation if needed
                    if (item.path === '/food/admin/orders') {
                      console.log('Mobile Orders navigation clicked');
                    }
                  }}
                >
                  {item.icon}
